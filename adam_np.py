...
def adam_optimizer(theta,gradient,m_pre,v_pre,t,alpha=0.001,beta1=0.9,beta2=0.999,epsilon=1e-8):
    ...
    v = (v_w,v_A,v_b)
    m_w_hat = m_w / (1.-beta1**t+epsilon)
    m_A_hat = m_A / (1.-beta1**t+epsilon)
    m_b_hat = m_b / (1.-beta1**t+epsilon)
    v_w_hat = v_w / (1.-beta2**t+epsilon)
    v_A_hat = v_A / (1.-beta2**t+epsilon)
    v_b_hat = v_b / (1.-beta2**t+epsilon)
